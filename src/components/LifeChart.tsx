...
export const LifeChart: React.FC<LifeChartProps> = ({ gameId }) => {
  ...
  if (!game || chartData.dataPoints.length === 0) {
    return (
      <div className="flex items-center justify-center py-8 text-center">
        ...
        <p className="text-muted-foreground">No life data to display</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 rounded-lg border bg-card p-4">
      <h3 className="text-lg font-semibold mb-4">Life Chart</h3>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3">
          {playerStats.players.map(({ name, finalLife, peak, lowest, netChange }) => (
            <div key={name} className="bg-secondary/50 rounded-lg p-3">
              ...
              <div className="mt-1 space-y-0.5">
                <p className="text-sm">
                  <span className="text-muted-foreground">Peak:</span> {peak} HP
                </p>
                <p className="text-sm">
                  <span className="text-muted-foreground">Lowest:</span> {lowest} HP
                </p>
              </div>
            </div>
          ))}
          {playerStats.biggestHit && (
            <div className="bg-secondary/30 rounded-lg p-3">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Biggest Hit</p>
              <p className="text-lg font-semibold">{playerStats.biggestHit.damage} damage</p>
              <p className="text-sm">
                {playerStats.biggestHit.from} <span className="text-muted-foreground">dealt to</span> {playerStats.biggestHit.to}{' '}
                <span className="text-muted-foreground">R{playerStats.biggestHit.round}T{playerStats.biggestHit.turn}</span>
              </p>
            </div>
          )}
        </div>
