      ...
      {showSuggestions && (suggestions.length > 0 || isLoading) && (
        <FadeMask showMask={suggestions.length > 2}>
          <div className="flex flex-col gap-1 max-h-58">
            {isLoading && (
              <div className="flex items-center justify-center gap-2 p-3 text-center text-muted-foreground">
                <div className="animate-spin inline-block w-4 h-4 border-2 border-muted border-t-primary rounded-full"></div>
                Searching...
              </div>
            )}

            {!isLoading && suggestions.length === 0 && newCommander.trim().length >= 2 && (
              <div className="p-3 text-center text-muted-foreground">No commanders found. Try a different search term.</div>
            )}
