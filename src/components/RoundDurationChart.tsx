...
export const RoundDurationChart: React.FC<RoundDurationChartProps> = ({ gameId }) => {
  ...
  if (!game || chartData.dataPoints.length === 0) {
    return (
      <div className="flex items-center justify-center py-8 text-center">
        ...
        <p className="text-base-content/70">No duration data to display</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 bg-base-200 rounded-lg border border-base-300 p-4">
      <h3 className="text-lg font-semibold text-base-content mb-4">Round Duration Chart</h3>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3">
          {playerStats.players.map(({ name, average, longest, shortest }) => (
            <div key={name} className="bg-base-300/50 rounded-lg p-3">
              ...
              <p className="text-lg font-semibold text-base-content">Avg: {formatDuration(average)}</p>
              <div className="mt-1 space-y-0.5">
                <p className="text-sm text-base-content/80">
                  <span className="text-base-content/70">Longest:</span> {formatDuration(longest.duration)}{' '}
                  <span className="text-base-content/60">
                    R{longest.round}T{longest.turn}
                  </span>
                </p>
                <p className="text-sm text-base-content/80">
                  <span className="text-base-content/70">Shortest:</span> {formatDuration(shortest.duration)}{' '}
                  <span className="text-base-content/60">
          ...
          {playerStats.longest && (
            <div className="bg-base-300/50 rounded-lg p-3">
              <p className="text-xs text-base-content/70 uppercase tracking-wide">Longest Turn</p>
              <p className="text-lg font-semibold text-base-content">{formatDuration(playerStats.longest.duration)}</p>
              <p className="text-sm text-base-content/80">
